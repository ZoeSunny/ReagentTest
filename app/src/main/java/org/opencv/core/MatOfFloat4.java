...
public class MatOfFloat4 extends Mat
{
    ...
    protected MatOfFloat4(final long n) {
        super(n);
    ...
    public MatOfFloat4(final Mat mat) {
        super(mat, Range.all());
        if (!this.empty() && this.checkVector(4, 5) < 0) {
            throw new IllegalArgumentException("Incompatible Mat");
        }
    ...
    public MatOfFloat4(final float... array) {
        this.fromArray(array);
    }
    
    public static MatOfFloat4 fromNativeAddr(final long n) {
        return new MatOfFloat4(n);
    }
    ...
    public float[] toArray() {
        final int checkVector = this.checkVector(4, 5);
        if (checkVector < 0) {
            throw new RuntimeException("Native Mat has unexpected type or size: " + this.toString());
        }
        final float[] array = new float[checkVector * 4];
        if (checkVector == 0) {
            return array;
        }
        this.get(0, 0, array);
